...
Grab `Game.Browser.Setup.<version>.exe` from the [latest release](https://github.com/limburatorul/gamebrowser/releases/latest) and run it. It installs for the current user only, so it never asks for administrator rights, and you can choose the folder.
